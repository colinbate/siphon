...
class soap_server extends nusoap_base {
	...
	*/
	function invoke_method() {
		...
		// does method exist?
		// EDIT: Colin Bate 2007-02-23
		$class = get_class(SiphonService::$target);
		...
		if ($class == '') {
			if (!function_exists($this->methodname)) {
				...
				$this->fault('Client',"method '$class/$this->methodname' not defined in service");
				return;
			}
		} else {
		...
		if (!function_exists('call_user_func_array')) {
			...
			if ($class == '') {
				...
				// EDIT: Colin Bate 2007-02-23
				$call_arg = array(SiphonService::$target, "$this->methodname");
				//$call_arg = "$this->methodname";	// straight assignment changes $this->methodname to lower case after call_user_func_array()
			} elseif ($delim == '..') {
				$this->debug('in invoke_method, calling class method using call_user_func_array()');
				// EDIT: Colin Bate 2007-02-23
				$call_arg = array(SiphonService::$target, $method);
		...
		// returned value(s)
			...
			if($this->wsdl){
				...
				if($errstr = $this->wsdl->getError()){
					...
					// ADDED BY COLIN BATE
					$fp = fopen('/tmp/colin-soap-debug.txt', 'w');
					fwrite($fp, $this->getDebug());
					fclose($fp);
					// -------------------
					return;
				}
...
class wsdl extends nusoap_base {
    ...
    */
    function webDescription(){
		...
		// ADDED BY COLIN BATE
		$add = '?wsdl';
		if (isset($_SERVER['QUERY_STRING']) && strpos($_SERVER['QUERY_STRING'], 'doc') !== false) {
			$add = '?wsdl&doc';
		}
		// ___________________
		if (isset($_SERVER)) {
		...
		$wsdl = $PHP_SELF . $add;
		$wadl = $PHP_SELF . '?wadl';
		$service =& SiphonService::getCurrent();
		$author = '<strong>Author:</strong> ';
		if ($service->author != '') {
			$author .= htmlspecialchars($service->author);
		} else {
			$author .= "None";
		}
		$package = '<strong>Package:</strong> ';
		if ($service->package != '') {
			$package .= htmlspecialchars($service->package);
		} else {
			$package .= 'unknown';
		}
		$operations = '<div id="oplist">';
		$opcount = 0;
		foreach($this->getOperations() as $op => $data) {
			$op_d = "{$op}_details";
			$operations .= "<div id=\"$op\">\n";
			
			$operations .= "<h3 class=\"opname\"><a href=\"javascript:return true;\" class=\"toggle\" onclick=\"toggleDetails('$op_d', this);return true;\">$op</a></h3>\n";
			$operations .= "<p class=\"description\">" . htmlspecialchars($data['documentation']) . "</p>\n";
		    // create hidden div
		    $operations .= "<div id=\"$op_d\" class=\"detailhide\">\n<table border=\"0\" class=\"detailtable\">\n";
		    foreach ($data as $donnie => $marie){ // loop through opdata
				if ($donnie == 'input' || $donnie == 'output'){ // show input/output data
				    $operations .= "<tr><th colspan=\"2\">".ucfirst($donnie).":</th></tr>\n";
				    foreach ($marie as $captain => $tenille){ // loop through data
						if ($captain == 'parts'){ // loop thru parts
						    $operations .= "<tr><th class=\"level2\">$captain:</th><td>\n";
			                //if(is_array($tenille)){
						    	foreach($tenille as $joanie => $chachi){
									$operations .= "$joanie: $chachi<br>\n";
						    	}
			        		//}
							$operations .= "</td></tr>\n";
						} else {
						    $operations .= "<tr><th class=\"level2\">$captain:</th><td>$tenille</td></tr>\n";
						}
				    }
				} else {
					if ($donnie != 'documentation')
						$operations .= "<tr><th>".ucfirst($donnie).":</th><td>$marie</td></tr>\n";
				}
		    }
			$operations .= "</table>\n</div>\n";
			$opcount++;
		...
		if ($opcount == 0) {
			$operations .= "<p style=\"color: #444\">There are no operations in this service.</p>";
		}
		$operations .= "</div>\n";
		ob_start();
		@include(SIPHON_BASE_PATH . 't/1/service.inc');
		$sp = ob_get_contents();
		ob_end_clean();
		
		$b = '
		<html><head><title>Siphon: '.$this->serviceName.'</title>
		<style type="text/css">
			...
			background-color: #444; width: 105%; margin-left: 0px;
		...
		<div class=content>
			...
			<div class=nav>
				<p>View the <a href="' . $PHP_SELF . $add . '">WSDL</a> for the service.
		...
		<div class="footer">Server Auto-discovery, Binding, XML-RPC, WADL and REST interface by Siphon<br>SOAP and WSDL handling by NuSOAP</div></body></html>';
		return $sp;
    }
